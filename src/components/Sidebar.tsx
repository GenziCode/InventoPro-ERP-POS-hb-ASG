...
const modules: { name: ModuleType; icon: string }[] = [
  ...
  { name: 'Purchase Orders', icon: 'purchase-orders' },
  { name: 'Payments', icon: 'payments' },
  { name: 'Expenses', icon: 'expenses' },
  { name: 'Cargo', icon: 'cargo' },
  { name: 'Reports', icon: 'reports' },
  { name: 'Memberships', icon: 'memberships' },
  { name: 'Discounts & Deals', icon: 'discounts' },
  { name: 'Ecommerce Sync', icon: 'ecommerce' },
...
const Sidebar: React.FC<SidebarProps> = ({ activeModule, setActiveModule, isOpen, setIsOpen }) => {
  ...
  return (
    <>
      ...
      <div className={`fixed top-0 left-0 h-full bg-white shadow-lg z-40 transition-transform duration-300 ease-in-out md:hidden ${isOpen ? 'translate-x-0' : '-translate-x-full'} w-64 flex flex-col`}>
